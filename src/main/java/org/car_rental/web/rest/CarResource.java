...
public class CarResource {
    ...
    @GetMapping("/cars")
    @Timed
    public ResponseEntity<List<CarDTO>> getAllCars(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("concerned-is-null".equals(filter)) {
            log.debug("REST request to get all Cars where concerned is null");
            return new ResponseEntity<>(carService.findAllWhereConcernedIsNull(),
                    HttpStatus.OK);
        }
